...
extension RoomH on Room {
  User? get me => users.firstWhereOrNull((e) => e.id == '0');

  User get otherUser => users.firstWhere((e) => e.id != '0');
  User get firstUser => users.first;
  User get secondUser => users.last;
...
extension UserH on User {
  String get name => '$firstName';
}

extension MessageH on Message {
  Widget latestMessage(Room room) {
    bool isRead = room.isRead;
    String message = '';
    dynamic icon;

    if (this is CustomMessage) {
      return 0.0.verticalSpace;
    } else if (this is FileMessage) {
      message = 'ملف';
      icon = Icons.file_copy;
    } else if (this is ImageMessage) {
      message = 'صورة';
      icon = Icons.image;
    } else if (this is TextMessage) {
      message = (this as TextMessage).text;
      icon = Icons.message;
    }
    ...
    return Row(
      children: [
        ImageMultiType(
          color: isRead ? Colors.grey : mainColor,
          url: icon,
          height: 40.0.r,
          width: 40.0.r,
        ),
        ...
        SizedBox(
          width: 100.0.w,
          child: DrawableText(
            text: message/*.substring(0,message.length<10?message.length:10)*/,
            selectable: false,
            matchParent: true,
            maxLines: 1,
            size: 14.0.sp,
