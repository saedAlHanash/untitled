...
class FirebaseChatCore {
  ...
  Future<types.Room> createRoom(
    ...
    final userIds = ['0', otherUser.id]..sort();
  ...
  Stream<List<types.Room>> rooms({
    ...
    return collection.snapshots().asyncMap(
      (query) {
        loggerObject.v(query.docs.length);
  ...
  Future<List<types.Room>> roomsGet({
    bool orderByUpdatedAt = false,
  }) async {
    final collection = orderByUpdatedAt
        ? getFirebaseFirestore()
  ...
  Future<List<types.Room>> filter({required String name}) async {
    final query = await FirebaseFirestore.instance
        .collection(config.roomsCollectionName)
        .where('userIds', arrayContains: name)
        .get();
  ...
  Future<void> latestSeenRoom(String roomId) async {
    await getFirebaseFirestore()
        ...
        .update(
      {
        'latestSeen0': FieldValue.serverTimestamp(),
      },
    );
  }
    ...
    return getFirebaseFirestore()
        ...
        .map(
          (snapshot) => snapshot.docs.fold<List<types.User>>(
            [],
            (previousValue, doc) {
              if ('0' == doc.id) return previousValue;
