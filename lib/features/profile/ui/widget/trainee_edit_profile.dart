import 'package:drawable_text/drawable_text.dart';
import 'package:fitness_storm/core/app/app_provider.dart';
import 'package:flutter/material.dart';
import 'package:flutter_screenutil/flutter_screenutil.dart';
import 'package:get/get.dart';
import 'package:image_multi_type/circle_image_widget.dart';
import 'package:image_multi_type/image_multi_type.dart';

import '../../../../Widgets/custom_text_field.dart';
import '../../../../generated/l10n.dart';

class TraineeEditProfile extends StatelessWidget {
  const TraineeEditProfile({super.key});

  @override
  Widget build(BuildContext context) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.center,
      children: [
        SizedBox(height: Get.height / 15.3),
        Container(
          width: Get.width / 4,
          height: Get.width / 4,
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(100),
            border: Border.all(width: 2, color: Colors.white),
            boxShadow: [
              BoxShadow(
                color: Colors.grey.withOpacity(0.5),
                spreadRadius: 0.1,
                blurRadius: 8,
                offset: const Offset(0, 10), // changes position of shadow
              ),
            ],
          ),
          child: CircleImageWidget(url: AppProvider.profile.image, size: 100.0.r),
        ),
        SizedBox(height: Get.height / 27),
        Text(
          AppProvider.profile.name ?? "no_name".tr,
          style: TextStyle(
              fontSize: 26, color: Get.theme.primaryColor, fontWeight: FontWeight.bold),
        ),
        30.0.verticalSpace,
        TextButton(
          onPressed: () {
            Get.dialog(
              AlertDialog(
                shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(12),
                ),
                title: Text(
                  'change_password'.tr,
                  style: TextStyle(
                    fontSize: 16,
                    color: Get.theme.primaryColor,
                  ),
                ),
                content: SingleChildScrollView(
                  child: Form(
                    child: Column(
                      children: [
                        CustomTextField(
                          'new_password'.tr,
                          controller: AppTextFieldController(false),
                          decoration:
                              BoxDecoration(color: Colors.white.withOpacity(0.15)),
                          // textController: controller.newPassword,
                          style: const TextStyle(color: Color(0xFF565C63), fontSize: 16),
                          isUnderline: true,
                          hint: "new_password".tr,
                        ),
                        CustomTextField(
                          'Confirm_Password'.tr,
                          controller: AppTextFieldController(false),
                          decoration:
                              BoxDecoration(color: Colors.white.withOpacity(0.15)),
                          // textController: controller.confirmationPassword,
                          style: const TextStyle(color: Color(0xFF565C63), fontSize: 16),
                          isUnderline: true,
                          hint: "Confirm_Password".tr,
                        ),
                      ],
                    ),
                  ),
                ),
                actions: [
                  MaterialButton(
                    onPressed: () {},
                    child: Text(
                      'change'.tr,
                      style: TextStyle(
                        fontSize: 12,
                        color: Get.theme.primaryColor,
                      ),
                    ),
                  ),
                ],
                // content: ,
              ),
            );
          },
          child: DrawableText(
            matchParent: true,
            drawableAlin: DrawableAlin.withText,
            text: S.of(context).changePassword,
            drawablePadding: 10.0.w,
            drawableEnd: const ImageMultiType(url: Icons.lock),
          ),
        ),
        SizedBox(height: Get.height / 27),
      ],
    );
  }
}
